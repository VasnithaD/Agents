...
export class LLMHandler {
  ...
  async generateCode(params: LLMGenerateCodeParams): Promise<MCPResponse> {
    try {
      ...
      const ragContext = await ragRetriever.retrieve(ragQuery, 5);

      const systemPrompt = this.buildGenerateSystemPrompt(language, params.context || '');
      ...
      if (isLargeTask(params.prompt)) {
        ...
        const structuredOutput = await this.normalizeGeneratedStructure(normalized, language, modelId, params.prompt);
  ...
  async refactorCode(params: LLMRefactorCodeParams): Promise<MCPResponse> {
    try {
      ...
      const userInstructions = params.instructions?.trim()
        || 'Apply all best-practice improvements.';
