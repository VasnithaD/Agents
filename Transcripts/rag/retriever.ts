 ...
 *
 *   initialize()   → load docs from disk, chunk them, build TF-IDF index
 *   retrieve(q, k) → return top-K chunks as a formatted context string
...
const CHUNK_SIZE   = parseInt(process.env.RAG_CHUNK_SIZE   || '600');   // chars
const CHUNK_OVERLAP= parseInt(process.env.RAG_CHUNK_OVERLAP|| '100');   // chars
...
function toAbsolutePath(p: string): string {
  ...
  return path.resolve(RAG_DOCUMENTS_PATH, clean);
}

function getRelatedFolders(paths: string[], limit = 3): string[] {
...
class RAGRetriever {
  ...
  private _formatNeural(results: VSearchResult[], _query: string): string {
    const relatedFolders = getRelatedFolders(results.map(r => r.chunk.source));
    const lines = [
      '### Relevant NGQ Project Context  (neural semantic search)',
      `*${results.length} passage(s) retrieved via embedding similarity.*`,
      `*RAG project root: ${RAG_DOCUMENTS_PATH}*`,
      '',
    ];

    if (relatedFolders.length > 0) {
      lines.push('### Related Folders From RAG');
      for (const folder of relatedFolders) lines.push(`- ${folder}`);
      lines.push('');
    }

    for (let i = 0; i < results.length; i++) {
      const { chunk, score } = results[i];
      const sourcePath = toAbsolutePath(chunk.source);
  ...
  private _formatTFIDF(results: SearchResult[]): string {
    const relatedFolders = getRelatedFolders(results.map(r => r.chunk.filePath));
    const lines = [
    ...
    for (let i = 0; i < results.length; i++) {
      const { chunk, score } = results[i];
      const sourcePath = toAbsolutePath(chunk.filePath);
